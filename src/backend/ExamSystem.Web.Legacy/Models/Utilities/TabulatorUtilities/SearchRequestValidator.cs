...
public class SearchRequestValidator<TModel> : AbstractValidator<SearchRequest> where TModel : class
{
    private readonly List<(string Name, Type Type)> _columnNames = new(typeof(TModel).GetProperties()
           .Select(a =>
           {
               ...
               {
                   return (a.Name, Type: Enum.GetUnderlyingType(a.PropertyType));
               }
        ...
        new Dictionary<OperatorType, IList<Type>>
        {
             ...
             },
             {
                 ...
                 new List<Type>
                 {
                     ...
                     typeof(long), typeof(Enum), typeof(Guid)
                 }
             },
             ...
             { OperatorType.EndsWith, new List<Type> { typeof(string) } },
             {
                 ...
                 new List<Type>
                 {
                     typeof(string), typeof(Guid), typeof(List<string>), typeof(List<int>), typeof(List<float>), typeof(List<double>),
    ...
    private bool IsValidValue(FilterColumn item)
    {
        try
        {
            ...
            {
                return Guid.TryParse(item.Value, out var _);
            }

            var _ = Convert.ChangeType(item.Value, column.Type, CultureInfo.InvariantCulture);
    ...
    private bool IsOperatorAllowed(FilterColumn item)
    {
        var column = _columnNames.FirstOrDefault(a => a.Name == item.FilterBy);
